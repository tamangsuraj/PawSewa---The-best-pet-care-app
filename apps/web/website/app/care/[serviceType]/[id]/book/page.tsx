    ...
    <div className="min-h-screen bg-white pb-32">
      <header className="sticky top-0 z-10 bg-white border-b border-gray-200">
        <div className="container mx-auto px-4 py-3 flex items-center gap-4">
          <Link href={`/care/${serviceType}/${params.id}`} className="p-2 -ml-2 rounded-lg hover:bg-gray-100 text-primary">
      ...
      <main className="container mx-auto px-4 py-6 space-y-6">
        <section>
          <h2 className="text-sm font-bold text-primary mb-2">SELECT PET</h2>
          <select
            value={selectedPetId}
        ...
        {isHostel ? (
          <>
            <section>
              <h2 className="text-sm font-bold text-primary mb-2">CHECK-IN / CHECK-OUT</h2>
            ...
            {center.roomTypes?.length ? (
              <section>
                <h2 className="text-sm font-bold text-primary mb-2">ROOM TYPE</h2>
                <div className="flex flex-wrap gap-2">
                  {center.roomTypes.map((r) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl border-2 text-sm ${roomType === r.name ? 'border-primary bg-primary/10 text-primary' : 'border-gray-200'}`}
                    >
          ...
          <>
            <section>
              <h2 className="text-sm font-bold text-primary mb-2">CHOOSE GROOMING PACKAGE</h2>
              <div className="grid grid-cols-2 gap-3">
                {PACKAGES.map((pkg) => (
                  <button
                    key={pkg.id}
                    ...
                    className={`p-4 rounded-xl border-2 text-left transition-all ${selectedPackage.id === pkg.id ? 'border-primary bg-primary/5' : 'border-gray-200 bg-white'}`}
                  >
            ...
            <section>
              <h2 className="text-sm font-bold text-primary mb-2">SERVICE DELIVERY</h2>
              <div className="grid grid-cols-2 gap-3">
                <button type="button" onClick={() => setDelivery('home')} className={`p-4 rounded-xl border-2 flex items-center gap-3 ${delivery === 'home' ? 'border-primary bg-primary/5' : 'border-gray-200'}`}>
                  <Home className="w-6 h-6 text-primary" />
                  <div className="text-left">
                    <p className="font-semibold text-primary">Home Visit</p>
                    <p className="text-xs text-gray-600">Groomer comes to your location</p>
                  </div>
                </button>
                <button type="button" onClick={() => setDelivery('center')} className={`p-4 rounded-xl border-2 flex items-center gap-3 ${delivery === 'center' ? 'border-primary bg-primary/5' : 'border-gray-200'}`}>
                  <Building2 className="w-6 h-6 text-primary" />
                  <div className="text-left">
                    <p className="font-semibold text-primary">Visit Center</p>
            ...
            <section>
              <h2 className="text-sm font-bold text-primary mb-2">ADD-ONS</h2>
              <div className="flex flex-wrap gap-2">
                {ADDONS.map((a) => (
                  <button
                    key={a.id}
                    ...
                    className={`px-4 py-2 rounded-full border-2 text-sm font-medium ${addonIds.includes(a.id) ? 'border-primary bg-primary text-white' : 'border-gray-200'}`}
                  >
                    {a.name} (+NPR {a.price})
                  </button>
                ))}
            ...
            <section>
              <h2 className="text-sm font-bold text-primary mb-2">SELECT DATE & TIME</h2>
              <div className="flex gap-2 overflow-x-auto pb-2">
                {days.map((d) => (
                  <button key={d.full.toISOString()} type="button" onClick={() => setSelectedDate(d.full)} className={`shrink-0 w-14 py-2 rounded-xl border-2 text-center ${selectedDate?.getDate() === d.date ? 'border-primary bg-primary text-white' : 'border-gray-200'}`}>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {TIME_SLOTS.map((t) => (
                  <button key={t} type="button" onClick={() => setSelectedTime(t)} className={`px-4 py-2 rounded-xl border-2 ${selectedTime === t ? 'border-primary bg-primary text-white' : 'border-gray-200'}`}>
                    {t}
                  </button>
                ))}
              </div>
        ...
        <section>
          <h2 className="text-sm font-bold text-primary mb-2">LOCATION & CONTACT</h2>
        ...
        <section>
          <h2 className="text-sm font-bold text-primary mb-2">PAYMENT & SUMMARY</h2>
          <div className="flex gap-2 mb-3">
            <input type="text" placeholder="Promo Code" value={promoCode} onChange={(e) => setPromoCode(e.target.value)} className="flex-1 px-4 py-2 border-2 border-gray-200 rounded-xl outline-none focus:border-primary" />
            <button type="button" className="px-4 py-2 bg-primary text-white rounded-xl font-medium">Apply</button>
          </div>
          <div className="flex gap-2 mb-3">
            {(['esewa', 'khalti', 'cod'] as const).map((method) => (
              <button key={method} type="button" onClick={() => setPaymentMethod(method)} className={`flex-1 py-2 rounded-xl border-2 font-medium uppercase ${paymentMethod === method ? 'border-primary bg-primary/10 text-primary' : 'border-gray-200'}`}>
          ...
          <div className="p-4 rounded-xl bg-gray-50 space-y-1 text-sm">
            {isHostel ? (
              <>
                ...
                <p className="flex justify-between font-bold text-primary text-base pt-2"><span>Total Amount</span><span>NPR {totalAmount.toLocaleString()}</span></p>
              </>
            ) : (
              <>
